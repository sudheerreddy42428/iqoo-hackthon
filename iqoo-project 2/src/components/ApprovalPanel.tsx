...
export const ApprovalPanel: React.FC<ApprovalPanelProps> = ({
  ...
  const handleRunAutoFix = () => {
    ...
    if (isHighRisk) {
      // Trigger developer report directly for high risk items
      onReject();
      return;
    }
  ...
  return (
    <div className="glass-panel rounded-xl overflow-hidden border border-amber-500/30 shadow-2xl animate-fadeIn mt-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              AI has prepared a patch. Please review the change location below before approving.
            </p>
          </div>
        </div>
      ...
      {approvalStatus === 'PENDING' && analysis.changeLocation && (
      ...
      {(testStatus === 'RUNNING' || patchStatus === 'APPLIED') && (
        <div className="p-4 sm:p-5 bg-dark-950 space-y-4">
          ...
          <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all duration-700 ${patchStatus === 'APPLIED' ? 'bg-emerald-500' : 'bg-gradient-to-r from-amber-500 to-indigo-400'}`}
