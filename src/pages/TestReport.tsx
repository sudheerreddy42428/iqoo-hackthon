...
export const TestReport: React.FC<TestReportProps> = ({ testId, onExit }) => {
  const { investigationState, activeCrash } = useInvestigation();
  const testCase = TEST_CASES.find(t => t.id === testId);

  const isSuccess = testCase?.scenario === 'HAPPY_PATH' || investigationState === 'RESOLVED';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn pb-20 mt-4">
      ...
      <div className={`p-8 rounded-2xl border ${isSuccess ? 'bg-emerald-950/20 border-emerald-500/30' : 'bg-rose-950/20 border-rose-500/30'} flex flex-col items-center text-center space-y-6`}>
        ...
        {activeCrash && investigationState === 'RESOLVED' && (
        ...
        <button
          onClick={onExit}
          className={`mt-4 px-8 py-3 rounded-xl font-bold transition-all shadow-lg ${
            isSuccess 
