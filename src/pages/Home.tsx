...
const HeroSection = ({ onLaunch }: { onLaunch: () => void }) => (
  <section className="relative pt-24 sm:pt-32 pb-16 sm:pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-12 animate-slide-up-fade w-full max-w-full">
    ...
    <div className="flex-1 space-y-6 sm:space-y-8 text-center lg:text-left relative z-10 w-full max-w-full">
      ...
      <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tighter text-white leading-[1.1] break-words">
      ...
      <p className="text-base sm:text-lg md:text-xl text-slate-400 max-w-2xl mx-auto lg:mx-0 leading-relaxed font-medium">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 sm:gap-4 pt-2">
        <button
          onClick={onLaunch}
          className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-white font-bold text-sm sm:text-base flex items-center justify-center gap-2 shadow-xl shadow-cyan-500/25 hover:shadow-cyan-500/40 hover:-translate-y-1 transition-all duration-300"
        >
        ...
        <button
          onClick={onLaunch}
          className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl bg-dark-900/80 hover:bg-dark-800 text-slate-300 hover:text-white border border-slate-700 font-semibold text-sm sm:text-base flex items-center justify-center gap-2 transition-all duration-300 hover:-translate-y-1"
        >
      ...
      <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 sm:gap-6 pt-2 text-xs font-mono text-slate-500">
        <div className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> 15 actions retained</div>
        <div className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> PII masked on-device</div>
        <div className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> Espresso + Compose tests</div>
      </div>
    </div>
...
const ProblemSection = () => (
  <section id="problem" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 relative w-full max-w-full">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 pt-4">
      {/* Left Card */}
      <div className="glass-panel p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl border border-rose-500/30 space-y-4 sm:space-y-6 relative overflow-hidden group hover:border-rose-500/50 transition-colors">
        ...
        <div className="flex items-center gap-3 text-rose-400 font-mono font-bold uppercase tracking-wider text-xs sm:text-sm">
      ...
      {/* Right Card */}
      <div className="glass-panel p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl border border-cyan-500/30 space-y-4 sm:space-y-6 relative overflow-hidden group hover:border-cyan-500/50 transition-colors">
        ...
        <div className="flex items-center gap-3 text-cyan-400 font-mono font-bold uppercase tracking-wider text-xs sm:text-sm">
          <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
          <span>What developers actually need to know</span>
        </div>
        <div className="space-y-3 sm:space-y-4 font-mono text-xs sm:text-sm text-slate-300">
          <div className="p-3 sm:p-4 rounded-xl bg-dark-950/80 border border-slate-800/80 flex items-center gap-3 sm:gap-4 shadow-inner">
            <span className="text-cyan-400 font-bold text-lg sm:text-xl shrink-0">?</span>
...
const HowItWorks = () => {
  const steps = [
    ...
    { num: '04', title: 'Contextual Analysis', desc: 'Stack frames and breadcrumbs are cross-referenced to identify likely cause.', icon: Cpu },
  ...
  return (
    <section id="how-it-works" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 sm:space-y-16 w-full max-w-full">
      ...
      <div className="relative pt-4 sm:pt-8">
        ...
        <div className="lg:hidden absolute top-[10%] bottom-[10%] left-[36px] sm:left-[47px] w-px bg-slate-800" />

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-6 relative z-10">
          {steps.map((step, idx) => {
            const Icon = step.icon;
            return (
              <div key={idx} className="flex lg:flex-col items-start lg:items-center gap-4 sm:gap-6 group">
                <div className="relative shrink-0">
                  <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-dark-900 border border-slate-700/80 flex items-center justify-center shadow-xl group-hover:border-purple-500/50 group-hover:shadow-purple-500/20 transition-all duration-300 group-hover:-translate-y-1 z-10 relative">
                    <Icon className="w-7 h-7 sm:w-10 sm:h-10 text-slate-400 group-hover:text-purple-400 transition-colors" />
                  </div>
                  <div className="absolute -top-2 -right-2 sm:-top-3 sm:-right-3 w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-purple-500 border-2 sm:border-4 border-dark-950 flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-white z-20 shadow-sm">
                    {step.num}
                  </div>
                </div>
                <div className="flex-1 lg:text-center pt-1 lg:pt-0 min-w-0">
                  <h4 className="font-bold text-white text-base sm:text-xl mb-1 sm:mb-3 tracking-tight">{step.title}</h4>
                  <p className="text-xs sm:text-[15px] text-slate-400 leading-relaxed font-medium">{step.desc}</p>
                </div>
              </div>
            );
          })}
  ...
  return (
    <section id="why-reprox" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 w-full max-w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 pt-4">
        ...
        <div className="glass-panel p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-[2rem] border border-rose-900/50 bg-rose-950/5 space-y-4 sm:space-y-6">
          <h4 className="text-rose-400 font-bold text-lg sm:text-xl mb-4 sm:mb-6">Traditional Workflow</h4>
          <ul className="space-y-3 sm:space-y-4 text-xs sm:text-sm">
            {traditional.map((step, idx) => (
              <li key={idx} className="flex items-center gap-3 sm:gap-4 text-slate-400">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-dark-900 border border-slate-800 flex items-center justify-center shrink-0 font-mono text-[9px] sm:text-[10px]">
                  {idx + 1}
                </div>
                <span>{step}</span>
              </li>
        ...
        {/* ReproX Workflow */}
        <div className="glass-panel p-6 sm:p-8 md:p-10 rounded-2xl sm:rounded-[2rem] border border-cyan-500/30 bg-indigo-950/10 space-y-4 sm:space-y-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/10 blur-[80px] rounded-full pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-purple-500/10 blur-[80px] rounded-full pointer-events-none" />
          
          <h4 className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent font-bold text-lg sm:text-xl mb-4 sm:mb-6">ReproX Workflow</h4>
          <ul className="space-y-3 sm:space-y-4 relative z-10 text-xs sm:text-sm">
            {reprox.map((step, idx) => (
              <li key={idx} className="flex items-center gap-3 sm:gap-4 text-slate-200 font-medium">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-tr from-cyan-500 to-purple-600 flex items-center justify-center shrink-0 font-mono text-[9px] sm:text-[10px] text-white">
                  {idx + 1}
                </div>
                <span>{step}</span>
              </li>
...
const FeatureGrid = () => (
  <section id="features" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 w-full max-w-full">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
      <div className="glass-panel p-6 sm:p-8 lg:p-10 rounded-2xl sm:rounded-3xl border border-slate-800/80 hover:border-cyan-500/40 transition-all duration-300 group hover:-translate-y-1 sm:hover:-translate-y-2 hover:shadow-2xl hover:shadow-cyan-900/20 space-y-4 sm:space-y-6">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 group-hover:scale-110 group-hover:bg-cyan-500/20 transition-all duration-300">
          <History className="w-6 h-6 sm:w-7 sm:h-7" />
        </div>
        <h4 className="font-bold text-xl sm:text-2xl text-white tracking-tight">15-Action Rolling Buffer</h4>
        <p className="text-xs sm:text-[15px] text-slate-400 leading-relaxed font-medium">
      ...
      <div className="glass-panel p-6 sm:p-8 lg:p-10 rounded-2xl sm:rounded-3xl border border-slate-800/80 hover:border-purple-500/40 transition-all duration-300 group hover:-translate-y-1 sm:hover:-translate-y-2 hover:shadow-2xl hover:shadow-purple-900/20 space-y-4 sm:space-y-6">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-purple-400 group-hover:scale-110 group-hover:bg-purple-500/20 transition-all duration-300">
          <Cpu className="w-6 h-6 sm:w-7 sm:h-7" />
        </div>
        <h4 className="font-bold text-xl sm:text-2xl text-white tracking-tight">Deterministic Analysis</h4>
        <p className="text-xs sm:text-[15px] text-slate-400 leading-relaxed font-medium">
          Cross-references stack frames with user breadcrumbs using rule-based fallbacks and optional LLM-powered analysis.
        </p>
      </div>

      <div className="glass-panel p-6 sm:p-8 lg:p-10 rounded-2xl sm:rounded-3xl border border-slate-800/80 hover:border-emerald-500/40 transition-all duration-300 group hover:-translate-y-1 sm:hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-900/20 space-y-4 sm:space-y-6">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 group-hover:scale-110 group-hover:bg-emerald-500/20 transition-all duration-300">
          <FileCode className="w-6 h-6 sm:w-7 sm:h-7" />
        </div>
        <h4 className="font-bold text-xl sm:text-2xl text-white tracking-tight">Automated Test Generation</h4>
        <p className="text-xs sm:text-[15px] text-slate-400 leading-relaxed font-medium">
  ...
  <section id="privacy" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full max-w-full">
    <div className="glass-panel p-6 sm:p-10 lg:p-16 rounded-2xl sm:rounded-[2.5rem] border border-emerald-500/20 bg-emerald-950/10 flex flex-col lg:flex-row items-center gap-8 sm:gap-16 relative overflow-hidden">
      ...
      <div className="flex-1 space-y-6 sm:space-y-8 relative z-10 w-full">
        <div className="flex items-center gap-3 text-emerald-400 text-xs sm:text-sm font-mono font-bold uppercase tracking-wider">
          <ShieldCheck className="w-5 h-5 sm:w-6 sm:h-6 shrink-0" />
          <span>Privacy by Design</span>
        </div>
        <h2 className="text-2xl sm:text-3xl md:text-5xl font-extrabold text-white tracking-tight leading-tight break-words">
          Sensitive data is scrubbed before it ever leaves the device.
        </h2>
        
        <ul className="space-y-3 sm:space-y-5 pt-2">
          {[
            ...
            <li key={i} className="flex items-start gap-3 sm:gap-4 text-slate-300 text-sm sm:text-lg">
              <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0 mt-0.5">
      ...
      <div className="flex-1 w-full max-w-full sm:max-w-lg relative z-10">
        <div className="p-5 sm:p-8 rounded-2xl sm:rounded-3xl bg-dark-950/80 border border-slate-800/80 shadow-2xl space-y-6 sm:space-y-8 font-mono text-xs sm:text-sm relative max-w-full">
          ...
          <div>
            <div className="text-slate-500 mb-2 sm:mb-3 font-semibold uppercase tracking-wider text-[11px] sm:text-xs">
              Before Masking <span className="text-rose-400 ml-2 border border-rose-500/30 bg-rose-500/10 px-1.5 py-0.5 rounded text-[10px]">Never stored</span>
            </div>
            <div className="text-slate-300 bg-slate-900/50 p-3 sm:p-4 rounded-xl border border-slate-800/80 shadow-inner break-all">
              Entered card <span className="text-rose-400 font-bold">4111-1111-1111</span>-1234
            </div>
          </div>

          <div className="flex justify-center -my-2 relative z-10">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-dark-950 border border-slate-800 flex items-center justify-center">
              <ArrowDownIcon className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500" />
            </div>
          </div>

          <div>
            <div className="text-slate-500 mb-2 sm:mb-3 font-semibold uppercase tracking-wider text-[11px] sm:text-xs">
              After Edge Masking <span className="text-emerald-400 ml-2 border border-emerald-500/30 bg-emerald-500/10 px-1.5 py-0.5 rounded text-[10px]">Stored in buffer</span>
            </div>
            <div className="text-emerald-300 bg-emerald-950/20 p-3 sm:p-4 rounded-xl border border-emerald-500/30 shadow-inner break-all">
  ...
  <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full max-w-full">
    <div className="relative text-center p-8 sm:p-12 md:p-20 rounded-2xl sm:rounded-[3rem] bg-gradient-to-br from-cyan-950/80 via-purple-950/80 to-indigo-950/80 border border-slate-700/50 shadow-2xl overflow-hidden group">
      ...
      <div className="space-y-6 sm:space-y-8 relative z-10">
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white tracking-tighter drop-shadow-sm break-words">
        ...
        <div className="pt-4 sm:pt-6">
          <button
            onClick={onLaunch}
            className="w-full sm:w-auto px-8 sm:px-12 py-3.5 sm:py-5 rounded-2xl bg-white text-dark-950 font-bold text-base sm:text-lg inline-flex items-center justify-center gap-3 shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300"
          >
...
const Footer = () => (
  <footer className="border-t border-slate-800 bg-dark-950 py-8 sm:py-12 px-4 sm:px-6 lg:px-8 w-full max-w-full">
    ...
    <div className="max-w-7xl mx-auto mt-6 sm:mt-8 text-xs text-slate-600 text-center md:text-left">
